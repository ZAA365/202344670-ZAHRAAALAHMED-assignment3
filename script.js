// 1. Add lazy loading to images (2 minutes)
<img src="assets/images/project1.png" loading="lazy" width="100" height="100">

// 2. Add visit timer (10 minutes)
let seconds = 0;
setInterval(() => {seconds++;
    document.getElementById('timer').textContent = 
        `Visit duration: ${Math.floor(seconds/60)}m ${seconds%60}s`;}, 1000);
...
// --- 2. THEME MANAGEMENT (From Assgn 2) ---
function initTheme() {
...
// --- 3. GREETING FEATURE (From Assgn 2) ---
...
// --- 4. VISIT TIMER (NEW Assgn 3) ---
var timerSeconds = 0;
function initTimer() {
    var saved = sessionStorage.getItem('portfolio-timer');
    if (saved) timerSeconds = parseInt(saved, 10);
    updateTimerDisplay();
    setInterval(function() {
        timerSeconds++;
        updateTimerDisplay();
        sessionStorage.setItem('portfolio-timer', timerSeconds.toString());
    }, 1000);
}
function updateTimerDisplay() {
    ...
    timerDisplay.textContent = '⏱ ' + (h > 0 ? h + 'h ' : '') + m + 'm ' + s + 's';
}

// --- 5. AUTH STATE MANAGEMENT (NEW Assgn 3) ---
function initAuth() {
    var savedUser = localStorage.getItem('portfolio-auth-user');
    if (savedUser) updateAuthUI(true, savedUser);
}
function handleLogin() {
    var user = authUsernameInput.value.trim();
    if (user === '') { authUsernameInput.style.borderColor = '#ff3b30'; return; }
...
function updateAuthUI(loggedIn, user) {
    if (loggedIn) {
        ...
        authBtn.onclick = function() { authPanel.classList.toggle('hidden'); };
    }
}

// --- 6. PROJECT FILTER & SORT (NEW Assgn 3) ---
function initProjectFilters() {
    ...
    sortSelect.addEventListener('change', function() { applyFilterAndSort(document.querySelector('.filter-btn.active').getAttribute('data-filter')); });
}
function applyFilterAndSort(filter) {
    ...
    var visRows = rows.filter(function(r) { return !r.classList.contains('hidden-row'); });
    if (sort === 'name-asc') visRows.sort((a,b) => a.getAttribute('data-name').localeCompare(b.getAttribute('data-name')));
    if (sort === 'name-desc') visRows.sort((a,b) => b.getAttribute('data-name').localeCompare(a.getAttribute('data-name')));
    visRows.forEach(function(r) { projectsTbody.appendChild(r); });
    noProjectsMsg.classList.toggle('hidden', visible > 0);
}

// --- 7. GITHUB API INTEGRATION (NEW Assgn 3) ---
function initGitHub() {
...
// --- 8. CONTACT FORM VALIDATION (NEW Assgn 3) ---
...
function initContactForm() {
    contactForm.addEventListener('submit', function(e) {
        ...
        if (validateName() & validateEmail() & validateMessage()) {
            ...
            contactForm.reset(); [contactName, contactEmail, contactMessage].forEach(el => el.classList.remove('invalid'));
...
document.addEventListener('DOMContentLoaded', function() {
    initTheme(); initGreeting(); initTimer(); initAuth(); initProjectFilters(); initGitHub(); initContactForm();
    themeToggle.addEventListener('click', toggleTheme);
    greetBtn.addEventListener('click', handleGreet);
    usernameInput.addEventListener('keydown', function(e) { if (e.key === 'Enter') greetBtn.click(); });
    authSubmitBtn.addEventListener('click', handleLogin);
    authCancelBtn.addEventListener('click', function() { authPanel.classList.add('hidden'); authUsernameInput.value = ''; authUsernameInput.style.borderColor = ''; });
    authUsernameInput.addEventListener('keydown', function(e) { if (e.key === 'Enter') authSubmitBtn.click(); });
});